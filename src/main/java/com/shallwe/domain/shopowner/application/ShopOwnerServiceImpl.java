...
public class ShopOwnerServiceImpl implements ShopOwnerService {
  ...
  public Message confirmPayment(UserPrincipal userPrincipal, Long reservationId) {
    ...
    @Override
    @Transactional
    public Message uploadShopOwnerIdentification(ShopOwnerIdentificationReq shopOwnerIdentificationReq, UserPrincipal userPrincipal) {
        ShopOwner shopOwner = shopOwnerRepository.findById(userPrincipal.getId())
                .orElseThrow(InvalidShopOwnerException::new);

        shopOwner.updateIdentification(AwsS3ImageUrlUtil.toUrl(shopOwnerIdentificationReq.getIdentification()));
        shopOwner.updateBusinessRegistration(AwsS3ImageUrlUtil.toUrl(shopOwnerIdentificationReq.getBusinessRegistration()));
